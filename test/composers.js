...
test("stampit({ composers() })", (t) => {
  ...
  const stamp = stampit({
    composers(...args) {
      ...
      t.ok(
        _.isPlainObject(args[0].composables[0]),
        "the composable is a descriptor",
      );
      t.ok(
        _.isArray(args[0].composables[0].composers),
  ...
  t.equal(stamp.compose.composers.length, 1, "should be single composer");
...
test("stampit({ composers() }) returned value replaces stamp", (t) => {
  ...
  t.equal(
    stamp,
    replacement,
    "composer returned value should replace original stamp",
  );
...
test("stampit({ composers() }) a non-stamp should be ignored", (t) => {
  ...
  t.notEqual(
    stamp,
    replacement,
    "composer returned value should not replace original stamp",
  );
...
test("stampit({ composers() }) returned value passed to the second composer", (t) => {
  ...
  t.equal(
    stamp2,
    replacement,
    "second composer should get first composer return value",
  );
    ...
    composers({ composables }) {
      ...
      if (run === 1) {
        t.equal(
          composables.length,
          1,
          "creating stamp should pass one composable",
        );
      }
      if (run === 2) {
        t.equal(
          composables.length,
          2,
          "inheriting stamp should pass two composables",
        );
